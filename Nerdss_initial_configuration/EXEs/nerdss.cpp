...
int main(int argc, char* argv[])
{
    ...
    // ************************************read kinetic_t.csv to kineitc_parm************************************

    int i = 0;

    std::ifstream myFile;

    myFile.open("kinetics_t.csv");

    std::string genelet_t, kAA_t, kAB_t, kBB_t,kAC_t, kBC_t, kCC_t,    kub_AA_t, kub_AB_t, kub_BB_t, kub_AC_t, kub_BC_t, kub_CC_t;
    ...
    {
    //Read time 
    getline(myFile, genelet_t, ',');
    if(genelet_t.size() == 0) break;
    params.t_series.push_back(std::stof(genelet_t));

    //Read k_AA
    getline(myFile, kAA_t, ',');
    params.k_AA.push_back(std::stof(kAA_t));

    //Read k_unbind
    getline(myFile, kub_AA_t, ','); 
    params.kub_AA.push_back(std::stof(kub_AA_t));
    std::cout << "k_ub_AA_t: " << kub_AA_t << std::endl;

    //Read k_AB
    getline(myFile, kAB_t, ','); //new value after ,
    params.k_AB.push_back(std::stof(kAB_t));

    //Read k_unbind
    getline(myFile, kub_AB_t, ','); 
    params.kub_AB.push_back(std::stof(kub_AB_t));
    //std::cout << "k_ub: " << k_ub_t << std::endl;

    //Read k_BB
    getline(myFile, kBB_t, ','); //new value after ,                                                                                                                   
    params.k_BB.push_back(std::stof(kBB_t));

    //Read k_unbind
    getline(myFile, kub_BB_t, ','); 
    params.kub_BB.push_back(std::stof(kub_BB_t));
    std::cout << "k_ub_BB_t: " << kub_BB_t << std::endl;

    //Read k_AC
    getline(myFile, kAC_t, ','); //new value after ,
    params.k_AC.push_back(std::stof(kAC_t));

    //Read k_unbind
    getline(myFile, kub_AC_t, ','); 
    params.kub_AC.push_back(std::stof(kub_AC_t));
    //std::cout << "k_ub: " << k_ub_t << std::endl;

    //Read k_BC
    getline(myFile, kBC_t, ','); //new value after ,
    params.k_BC.push_back(std::stof(kBC_t));

    //Read k_unbind
    getline(myFile, kub_BC_t, ','); 
    params.kub_BC.push_back(std::stof(kub_BC_t));
    //std::cout << "k_ub: " << k_ub_t << std::endl;

    //Read k_CC
    getline(myFile, kCC_t, ','); //new value after ,                                                                                                                   
    params.k_CC.push_back(std::stof(kCC_t));


    //Read k_unbind
    getline(myFile, kub_CC_t, '\n'); 
    params.kub_CC.push_back(std::stof(kub_CC_t));
    std::cout << "kub_CC: " << kub_CC_t << std::endl;
    ...
    // ************************************create reaction index lists  *************************************

    params.rxn_AA.resize(78);
    std::iota(std::begin(params.rxn_AA), std::end(params.rxn_AA), 0); // Fill with 0, 1, ..., 78.
    std::cout << "indrxn_AA: " << std::endl;  
    for (int i : params.rxn_AA) {
    std::cout << i << ' '; // will print: "a b c"
    }

    params.rxn_AB.resize(144);
    std::iota(std::begin(params.rxn_AB), std::end(params.rxn_AB), 78); // Fill with 0, 1, ..., 144.
    std::cout << "indrxn_AB: " << std::endl;  
    for (int i : params.rxn_AB) {
    std::cout << i << ' '; // will print: "a b c"
    }

    params.rxn_BB.resize(78);
    std::iota(std::begin(params.rxn_BB), std::end(params.rxn_BB), 78+144); // Fill with 0, 1, ..., 78.
    std::cout << "indrxn_BB: " << std::endl;  
    for (int i : params.rxn_BB) {
    std::cout << i << ' '; // will print: "a b c"
    }
 
    params.rxn_AC.resize(144);
    std::iota(std::begin(params.rxn_AC), std::end(params.rxn_AC), 78+144+78); // Fill with 0, 1, ..., 78.
    std::cout << "indrxn_AC: " << std::endl;  
    for (int i : params.rxn_AC) {
    std::cout << i << ' '; // will print: "a b c"
    }

    params.rxn_BC.resize(144);
    std::iota(std::begin(params.rxn_BC), std::end(params.rxn_BC), 78+144+78+144); // Fill with 0, 1, ..., 144.
    std::cout << "indrxn_BC: " << std::endl;  
    for (int i : params.rxn_BC) {
    std::cout << i << ' '; // will print: "a b c"
    }

    params.rxn_CC.resize(78);
    std::iota(std::begin(params.rxn_CC), std::end(params.rxn_CC), 78+144+78+144+144); // Fill with 0, 1, ..., 78.
    std::cout << "indrxn_CC: " << std::endl;  
    for (int i : params.rxn_CC) {
    std::cout << i << ' '; // will print: "a b c"
    }   
    ...
    //box_z
    params.box_z=membraneObject.waterBox.z;
    ...
    // ************************************read positons.csv to initial postion ************************************

    

    std::ifstream positionFile;

    positionFile.open("positions.csv");
    std::cout << "opening positions " << std::endl; 
    
    
    std::string strcomIdx,strcomX, strcomY, strcomZ ;
    std::cout << "define string " << std::endl; 
    
    int comIdx;
    comIdx=-1;  
    double comX, comY, comZ;
    std::cout << "create int comIdx, double com " << std::endl; 
    ...
    while (!positionFile.eof()) //while the end of file is NOT reached
    ...
    //Read index of particle
    //getline(positionFile, strcomIdx, ',');
    std::cout << "comIdx"<< comIdx<< std::endl;

    //Read x y z of particle
    getline(positionFile, strcomX, ',');
    if(strcomX.size() == 0) break;
    comX=std::stod(strcomX);

    getline(positionFile, strcomY, ',');
    comY=std::stod(strcomY);
    
    getline(positionFile, strcomZ,  '\n');
    std::cout << "strcomX"<<strcomX<< std::endl;;
    comZ=std::stod(strcomZ);
    ...
    moleculeList[comIdx].comCoord.x=comX;
    moleculeList[comIdx].comCoord.y=comY;
    moleculeList[comIdx].comCoord.z=comZ;
    //interface
    for(auto& interface : moleculeList[comIdx].interfaceList){
        interface.coord.x=comX;
        interface.coord.y=comY;
        interface.coord.z=comZ;
        std::cout << "comX"<< comX;
    ...
    else std::cout << "Unable to open position file"; //if the file is not open output
    


    for(auto& complex : complexList){
        complex.update_properties(moleculeList,molTemplateList);
    }
    /////Diffusing field 3D 
    //double field [101][101][101];
    ...
    for (simItr += 1; simItr < params.nItr; ++simItr) {
        ...
        if (params.pdbWrite != -1) {
            if (simItr % params.pdbWrite == 0) {
                ...
                //std::cout<<"to write complex components"<<std::endl;
                write_complex_component(simItr, simItr, params,  moleculeList, complexList, membraneObject);
            }
        }
