    ...
    const std::vector<Molecule>& moleculeList, const std::vector<MolTemplate>& molTemplateList)
{
    ...
    //define a longest vector
    Vector long_vector_n;
    ...
    for (auto& memMol : memberList) {
        ...
        if ((distVec.magnitude + molTemplateList[moleculeList[memMol].molTypeIndex].radius) > radius){
            ...
            long_vector_n = distVec/distVec.magnitude;}
    }

    radius_short= 0;
    for (auto& memMol : memberList) {
        Vector distVec_2 { moleculeList[memMol].comCoord - this->comCoord };
        distVec_2.calc_magnitude();
        double a = distVec_2.magnitude; 
        double b = distVec_2.dot(long_vector_n);
        double c = sqrt(a*a-b*b);
        if ((c + molTemplateList[moleculeList[memMol].molTypeIndex].radius) > radius_short){
            radius_short = c+ molTemplateList[moleculeList[memMol].molTypeIndex].radius;
            }
    }
    ...
    double box_v=0, sphere_v=0;
    int count=0;
    for (auto& memMol : memberList) {
    count+=1;
    Vector distVec_2 { moleculeList[memMol].comCoord - this->comCoord };
    if (distVec_2.x > x_max)x_max=distVec_2.x;
    if (distVec_2.x < x_min)x_min=distVec_2.x;
    if (distVec_2.y > y_max)y_max=distVec_2.y;
    if (distVec_2.y < y_min)y_min=distVec_2.y;
    if (distVec_2.z > z_max)z_max=distVec_2.z;
    if (distVec_2.z < z_min)z_min=distVec_2.z;
    }
    box_v=(x_max-x_min+1000)*(y_max-y_min+1000)*(z_max-z_min+1000)+0.00001;
    sphere_v= count*(4.0/3.0)*M_PI* pow(500.0,3);
    compactness=sphere_v/(box_v);
    if (count == 1.0)compactness=0.0;
