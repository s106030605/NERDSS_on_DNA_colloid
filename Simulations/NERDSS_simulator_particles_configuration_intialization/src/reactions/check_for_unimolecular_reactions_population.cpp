    ...
    std::map<std::string, int>& observablesList, copyCounters& counterArrays, Membrane& membraneObject) //
{
    ...
    for (auto& oneRxn : createDestructRxns) {
        ...
        if (oneRxn.rxnType == ReactionType::destruction) {
            ...
            if (oneTemp.isImplicitLipid == false) { // A is not implicit lipid
                ...
                while (rNum > prob) {
                    ++numEvents;
                    std::cout <<  " 1st "  << numEvents<< std::endl;
                ...
                while (rNum > prob) {
                    ++numEvents;
                    std::cout <<  " 2nd "  << numEvents<< std::endl;
                    prob += (exp(-lambda) * pow(lambda, numEvents)) / MathFuncs::gammFactorial(numEvents);
                }
                if (numEvents > NA) {
    ...
    for (auto& oneRxn : createDestructRxns) {
        ...
        if (oneRxn.rxnType == ReactionType::uniMolCreation) {
            ...
            while (rNum > prob) {
                ++numEvents;
                std::cout <<  " 3rd "  << numEvents<< std::endl;
                prob += (exp(-lambda) * pow(lambda, numEvents)) / MathFuncs::gammFactorial(numEvents);
            }
        ...
        if (forwardRxns[oneRxn.conjForwardRxnIndex].rxnType == ReactionType::bimolecular) {
            ...
            if (isImplicit == false) {
                ...
                if (NAB > 0) { // determine the reaction rate, now only work for all offrates of one backRxn are the same; TODO: extend this to the general case is tricky, for we need to track the boundPairs including required ancillary interfaces
                    ...
                    /////////////////////////////      Change unbinding kinetics ////////////////////////////////////////////////
            
                    /*
                    if (simItr<1000.0){
                        rate=0.0;
                        std::cout << "ub_rate: " << rate << std::endl;
                    }   else if (simItr>100.0*1000.0 && simItr<100.0*2000.0){
                        rate=rate*(simItr-(100.0*1000.0))/(100.0*1000.0);
                    }   

                    //if (simItr>100.0*100.0 && simItr<100.0*200.0){
                    //    rate=0.0;
                    //} 

                    //if (simItr>100.0*100.0 && simItr<100.0*300.0){
                    //    rate=0.0;
                    //} 
                    */

                    //forwardRxns[rxnIndex]
                    // I want to change k_ub based on RxnIndex   I can't use RxnIndex here. I try to put in void. Should I use oneRxn.absRxnIndex? (But, it seems not working.)

                    /// if rxnIndex fall in rxn_AA
                    if (std::find(params.rxn_AA.begin(), params.rxn_AA.end(), oneRxn.relRxnIndex) != params.rxn_AA.end() ) {
                        int index = floor(simItr*params.timeStep/1e6);   // ;  Simit 10ms   (SimItr/100) ; Simit 5ms   (SimItr/200)  ; Simit 2ms   (SimItr/500)   ;Simit 0.5ms ((SimItr/2000)) params.timeStep;
                        rate = params.kub_AA[index];
                        //std::cout << oneRxn.relRxnIndex << std::endl; 
                        //std::cout << rate << std::endl; 
                        //std::cout << "AA" << std::endl;   

                    }else if (std::find(params.rxn_AB.begin(), params.rxn_AB.end(), oneRxn.relRxnIndex) != params.rxn_AB.end() ) {
                        int index = floor(simItr*params.timeStep/1e6);   //Simit 1ms    index 1s 
                        rate = params.kub_AB[index];

                    }else if (std::find(params.rxn_BB.begin(), params.rxn_BB.end(), oneRxn.relRxnIndex) != params.rxn_BB.end() ) {
                        int index = floor(simItr*params.timeStep/1e6);   //Simit 1ms    index 1s 
                        rate = params.kub_BB[index];
                        //std::cout << oneRxn.relRxnIndex << std::endl; 
                        //std::cout << rate << std::endl; 
                        //std::cout << "BB" << std::endl;     

                    }else if (std::find(params.rxn_AC.begin(), params.rxn_AC.end(), oneRxn.relRxnIndex) != params.rxn_AC.end() ) {
                        int index = floor(simItr*params.timeStep/1e6);   //Simit 1ms    index 1s 
                        rate = params.kub_AC[index];
                    
                    }else if (std::find(params.rxn_BC.begin(), params.rxn_BC.end(), oneRxn.relRxnIndex) != params.rxn_BC.end() ) {
                        int index = floor(simItr*params.timeStep/1e6);   //Simit 1ms    index 1s 
                        rate = params.kub_BC[index];
                        //std::cout << "BC unbind" << oneRxn.relRxnIndex<< rate  << std::endl; 
                        

                    }else if (std::find(params.rxn_CC.begin(), params.rxn_CC.end(), oneRxn.relRxnIndex) != params.rxn_CC.end() ) {
                        int index = floor(simItr*params.timeStep/1e6);   //Simit 1ms    index 1s 
                        rate = params.kub_CC[index];
                        //std::cout << oneRxn.relRxnIndex << std::endl; 
                        //std::cout << rate << std::endl; 
                    ...
                    //int index = floor(simItr/1000.0);   //Simit 1ms    index 1s 
                    //rate = params.kub_AA[index];
                    //std::cout << "theory ub_rate: " << params.k_ub[index] << std::endl;
                    //std::cout << "real ub_rate: " << rate << std::endl;
                    /////////////////////////////      Change unbinding kinetics ////////////////////////////////////////////////
                    ...
                    while (rNum > prob) {
                        ++numEvents;
                        std::cout <<  " 4th "  << numEvents<< std::endl;
                        prob += (exp(-lambda) * pow(lambda, numEvents)) / MathFuncs::gammFactorial(numEvents);
                        std::cout   << prob<< std::endl;
                        
                        if (numEvents > NAB-1) break;
                    }
                    ...
                    if (numEvents > 0) {
                        ...
                        while (numEvents > 0) {
                            double ran_save = rand_gsl(); 
                            while (ran_save>1.0){
                                gsl_rng_set(r, 287);
                                ran_save = rand_gsl(); 
                                std::cout <<"ran_save"  << ran_save << std::endl;
                            }
                            int randIntNum { static_cast<int>(1.0 * NAB * ran_save) };
                            std::cout << "NAB"  << NAB << std::endl;
                            std::cout <<"ran_save"  << ran_save << std::endl;
                            std::cout <<"randIntNum" << randIntNum << std::endl;
                            if (randIntNum == NAB) {
                                randIntNum = NAB - 1;
